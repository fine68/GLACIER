            ...
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    if args.output_attention:
                        outputs,_ = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                    else:
...
def load_content(args):
    # Optional external prompt text. GLACIER uses its built-in domain
    # description when --prompt_domain is 0, so a missing prompt file is fine.
    file = 'ETT' if 'ETT' in args.data else args.data
    path = './dataset/prompt_bank/{0}.txt'.format(file)
